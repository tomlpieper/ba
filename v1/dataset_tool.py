...
class DatasetTool:
    ...
    @sleep_and_retry
    @limits(calls=1000, period=60)  # 60 requests per minute
    ...
    def preprocess_data(self, data, size: int) -> pd.DataFrame:
        ...
        if not self.v2:
            ...
            df['prompt'] = df.apply(self.generate_prompt, axis=1)
    ...
    def generate_prompt(self, row):
        match row['label']:
            case "Entailment":
                return f"Explain why this text \n\n{row['premise']}\n\nentails the following hypothesis: \n\n{row['hypothesis']}\n\nin 100 words or less."
            case "Neutral":
                return f"Explain why this text \n\n{row['premise']}\n\nis neutral to the following hypothesis: \n\n{row['hypothesis']}\n\nin 100 words or less."
            case "Contradiction":
                return f"Explain why this text \n\n{row['premise']}\n\ncontradicts the following hypothesis: \n\n{row['hypothesis']}\n\nin 100 words or less."
            case _:
                logger.error(f"Label '{row['label']}' not recognized.")
                return None  # Or handle the unknown label differently
