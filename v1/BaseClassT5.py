...
class BaseClassT5:   
    ...
    def load_local_dataset(self, dataset_name: str, splits: [str], path: str) -> None:
        ...
        try:
            ...
            # Shard the dataset
            # dataset_sharded = datasets['train_r1'].shard(num_shards=world_size, index=current_rank)
            # datasets = split_dataset_by_node(datasets, rank=int(os.environ["RANK"]), world_size=int(os.environ["WORLD_SIZE"]))
            # Access splits
